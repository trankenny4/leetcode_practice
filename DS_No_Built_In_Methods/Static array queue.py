...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        """Insert value into back of queue."""

        # expand array if needed
        if self._current_size == self._sa.length():
            self._double_queue()

        # increment _back before inserting the value
        self._back = self._increment(self._back)
        self._sa[self._back] = value
        self._current_size += 1

    def dequeue(self) -> object:
        """Remove and return least recently inserted value."""

        if self.is_empty():
            raise QueueException

        # move _front forward (or wraparound to index 0) and decrement size
    ...
    def front(self) -> object:
        """Return least recently value without removing it."""
    ...
    def _double_queue(self) -> None:
        """Resize StaticArray and reindex values."""

        new_array = StaticArray(self._sa.length() * 2)

        # use _front to place items in order in the new array
        for i in range(self._current_size):
            new_array[i] = self._sa[self._front]
            self._front = self._increment(self._front)

        # save array and make _front and _back really point to both ends
        self._sa = new_array
        self._front = 0
